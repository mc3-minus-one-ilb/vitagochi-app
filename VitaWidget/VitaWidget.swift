...
struct Provider: TimelineProvider {
    ...
    private func getData() throws -> ChallangeEntity? {
        ...
        return coreData.todayChallenge
    }
    ...
    init(entry: Provider.Entry) {
        ...
        if let records = entry.challange?.records?.allObjects as? [MealRecordEntity] {
            for record in records {
                if record.timeStatus == entry.timePhase.rawValue, self.isComplete {
                    break
                }
            }
        }
    ...
    init(entry: Provider.Entry) {
        ...
        if entry.timePhase == .afterDay {
            if mealCount == 3 {
                ...
                self.mediumMessage = "Cool! Just a ‘lil bit more step to" +
                "have better dietary habit ☺️"
                self.mood = .idle
            } else if mealCount == 1 {
